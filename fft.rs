...
//     let ptr_x: &mut [f32] = &mut ptr_scratch[0..2048];
//    // let ptr_y: &mut [f32] = &mut ptr_scratch[2048..];
//     let ptr_y: &mut [f32] = &mut ptr_scratch[2048..];
    let ptr_x: &mut [f32];
    let ptr_y: &mut [f32];
    {
        let (left, right) = ptr_scratch.split_at_mut(2048);
        ptr_x = left;
        ptr_y = right;
    }
    let ptr_x_index: usize = 0;
    let mut ptr_y_index: usize = 0;
    ...
    for _i in (1..n_stages).rev() {
        ...
        let mut data: Vec<f32> = ptr_y.to_vec();
        //println!("{}", data[data_index]);
        ...
        for _k in (1..=in_loop_cnt).rev() {
            ...
            data[data_index] = x0r;  ptr_y[data_index] = x0r;
            data[data_index + 1] = x0i; ptr_y[data_index + 1] = x0i;
            data_index += (del << 1) as usize;  

            //println!("{}", data[data_index]);
            data[data_index] = x2r;  ptr_y[data_index] = x2r;
            data[data_index + 1] = x2i;  ptr_y[data_index + 1] = x2i;
            data_index += (del << 1) as usize;

            //println!("{}", data[data_index]);
            data[data_index] = x1r; ptr_y[data_index] = x1r;
            data[data_index + 1] = x1i;   ptr_y[data_index + 1] = x1i;
            data_index += (del << 1) as usize;

            //println!("{}", data[data_index]);
            data[data_index] = x3i;  ptr_y[data_index] = x3i; 
            data[data_index + 1] = x3r;  ptr_y[data_index + 1] = x3r;
            data_index += (del << 1) as usize;
        ...
        while j <= (nodespacing * del) >> 1
        {
            ...
            // twiddles += nodespacing;
            //println!("{:.6} {:.6} {:.6} {:.6} {:.6} {:.6}", w1, w2, w3, w4, w5, w6);
    ...
    if not_power_4 {
        ...
        for _j in (1..=(del / 2)).rev() {
            ...
            let w1 = twiddles[tw_index];
            let w4 = twiddles[tw_index + 257];
            let tmp: f32;
            tw_index += nodespacing as usize;

            x0r = ptr_y[ptr_y_index];
            x0i = ptr_y[ptr_y_index + 1];
            ptr_y_index += (del << 1) as usize;

            x1r = ptr_y[ptr_y_index];
            x1i = ptr_y[ptr_y_index + 1];

            tmp = ia_sub_flt(ia_mul_flt(x1r, w1), ia_mul_flt(x1i, w4));
            x1i = ia_mac_flt(ia_mul_flt(x1r, w4), x1i, w1) as f32;
            x1r = tmp;

            ptr_y[ptr_y_index] = ia_sub_flt(x0r, x1r);
            ptr_y[ptr_y_index + 1] = ia_sub_flt(x0i, x1i);
            ptr_y_index -= (del << 1) as usize;

            ptr_y[ptr_y_index] = ia_add_flt(x0r, x1r);
            ptr_y[ptr_y_index + 1] = ia_add_flt(x0i, x1i);
            ptr_y_index += 1;
    ...
    for i in 0..n_points {
        ptr_real[i as usize] = ptr_y[(2 * i) as usize];
        ptr_imag[i as usize] = ptr_y[(2 * i + 1) as usize];
        //println!("{}  {}", ptr_real[i as usize] ,ptr_imag[i as usize]);
    }  
    //println!("{:?}, {:?}", ptr_real, ptr_imag);
}
